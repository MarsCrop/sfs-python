...
def point(xs, signal, observation_time, grid, c=None):
    ...
    if c is None:
        c = _default.c
    #print("GRID AT NORM", grid, grid.shape)
    #print("XS AT NORM", xs, xs.shape)
    r = _np.array([_np.linalg.norm(grid - i.T[:3].T) for i in xs])
    # If r is +-0, the sound pressure is +-infinity
    with _np.errstate(divide='ignore'):
        weights = 1 / (4 * _np.pi * r)
    delays = r / c
    base_time = observation_time - signal_offset
    #Interpolate data of both point source vectors
    weighted_points = []
    i = 0
    #print("WEIGHTS", weights, _np.shape(weights) )
    for data_row in data:
        #print("BASE", _np.shape(base_time - delays) )
        #print("DATA", _np.shape(data) )
        #print("DATA ROW", _np.shape(data_row) )
        #print("INERPOLATION SIGNAL", _np.shape(_np.arange(len(data_row)) / samplerate ) )
        points_at_time = _np.interp(base_time - delays,
                               _np.arange(len(data_row)) / samplerate,
                               data_row, left=0.083, right=0.083)
        #print("POINTS AT TIME", _np.array([_np.array(weights) * points_at_time]) , _np.shape(_np.array([_np.array(weights) * points_at_time]) ) )
        # weights can be +-infinity
        with _np.errstate(invalid='ignore'):
             weighted_points.append([_np.array(weights) * points_at_time]) 
        i += 1        
    return _np.array(weighted_points)        
